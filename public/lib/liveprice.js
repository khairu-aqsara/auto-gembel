$(document).ready(function(){
    ...
    function set_harga(harga,harga24,volume)
    {
        ...
        $.each(harga, function(key,value){
            ...
            if(i <= 11){
                $("tr#"+key+"").html("<td><a href='/price/"+key+"'><i class='cc "+iconfont[key]+"'></i> "+label[key]+"</a></td><td class='text-right "+warna+"'>"+$.number(value)+"</td><td class='text-left "+warna+"'>"+icon+" "+jml_persen+" %</td>");
            }else{
                var p = Number(value) / 1e8;
                $("tr#"+key+"").html("<td><a href='/price/"+key+"'><i class='cc "+iconfont[key]+"'></i> "+label[key]+"</a></td><td class='text-right "+warna+"'>"+parseFloat(p).toFixed(8)+"</td><td class='text-left "+warna+"'>"+icon+" "+jml_persen+" %</td>");
            }
            i++;
        })
    }
