$(document).ready(function(){
    ...
    function live_harga(data)
    {
        ...
        var change = parseFloat(Math.round((data.prices[key] - data.prices_24h[key]) / data.prices_24h[key] * 1000) / 10).toFixed(2);     
    ...
    $.post('https://api2.bitcoin.co.id/api/chartdata/'+key+'', {start: Date.now()-86400*1000, end: Date.now()}, function (data) {
        var ohlc = [], volume = [], dataLength = data.chart.length;
        var chartData = data.chart;
